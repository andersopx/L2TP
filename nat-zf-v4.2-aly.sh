...
print_rules_table() {
  load_interfaces
  echo "编号 | 客户入口UDP(公网优先) | 服务端UDP | 出口源IP | UDP状态 | 返回流量 | 去目标流量 | 总流量 | 出口网卡 | 下一跳 | 备注"
  subline 196
  local found=0
  while IFS='|' read -r id bind_ip listen_port target_ip target_port snat_ip remark _; do
    ...
    echo "$id | $(format_entry_endpoint "$bind_ip" "$listen_port") | ${target_ip}:${target_port} | $(format_ip_with_public_hint "$snat_ip") | ${udp_status} | $(format_bytes "$up_bytes") | $(format_bytes "$down_bytes") | $(format_bytes "$total_bytes") | ${dev:-未知} | ${via:-直连} | ${remark:--}"
  done < "$RULES_FILE"
  if [[ $found -eq 0 ]]; then
    echo "暂无规则"
  fi
